...
export const helperFunctions = {
   ...
   * @param {Object} snapshot - Game state snapshot
   * @param {Object} staticData - Static game data containing logic mappings
   * @param {Array} regionTuple - Array containing [fromRegion, toRegion]
  ...
  region_connection_rule(snapshot, staticData, regionTuple) {
    ...
    for (const requiredItems of possibleAccessMethods) {
      // Special case: if the requirement is "CANNOT ACCESS", this connection is blocked
      if (requiredItems.length === 1 && requiredItems[0] === 'CANNOT ACCESS') {
        return false;
      }
