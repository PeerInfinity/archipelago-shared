...
export function traverse(snapshot, staticData, doorName) {
  // Check if door color data is available
  if (!staticData || !staticData.doors) {
    console.warn(`[traverse] No door data available in staticData for door: ${doorName}`);
    return { bool: true, difficulty: 0 };  // Default to passable if no data
  }

  // Get player 1's door data (assuming single player for now)
  const playerDoors = staticData.doors['1'] || staticData.doors[1];
  if (!playerDoors) {
